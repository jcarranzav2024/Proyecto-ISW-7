...
class Docente {
  ...
  async Agregar(req, res) {
    ...
    try {
      ...
      res.json({ message: 'El docente fue agregado exitosamente.', resultado });
    } catch (error) {
      if (error.code === 'P2002') {
        // Prisma error de clave duplicada
        const campo = error.meta?.target?.[0];
        res.status(400).json({ error: `Ya existe un docente con el mismo valor en el campo: ${campo}.` });
      } else {
        console.error(`Error al agregar docente:`, error);
        res.status(500).json({ error: 'Ocurrió un error inesperado al agregar el docente.' });
      }
    }
  }
  ...
  async Actualizar(DocenteId, req, res) {
    ...
    try {
      ...
      if (!docenteExistente) {
        return res.status(404).json({ error: `No se encontró un docente con ID ${DocenteId}.` });
      }
      ...
      if (error.code === 'P2002') {
        const campo = error.meta?.target?.[0];
        res.status(400).json({ error: `Ya existe otro docente con el mismo valor en el campo: ${campo}.` });
      } else {
        console.error(`Error al actualizar docente:`, error);
        res.status(500).json({ error: 'Ocurrió un error inesperado al actualizar el docente.' });
      }
    }
  }

  async Borrar(DocenteId, req, res) {
    ...
    try {
      const resultado = await prisma.$transaction(async (prisma) => {
        ...
        return { message: 'El docente fue eliminado exitosamente.' };
      });

      res.json(resultado);
    } catch (error) {
      console.error(`Error al borrar docente ${DocenteId}:`, error);
      res.status(500).json({ error: 'No se pudo eliminar el docente. Verifique si está relacionado con otros datos.' });
    }
  }
}
